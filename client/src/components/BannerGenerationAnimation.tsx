...
type Phase = "dissolve" | "arrange" | "wait" | "form" | "complete";
...
export default function BannerGenerationAnimation({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      /* ─── FORM ─── particles fly from grid to output positions ─── */
      if (ph === "form") {
        const raw = Math.min(dt / FORM_MS, 1);
        const e = easeIO4(raw);

        for (const p of pts) {
          const nx = mix(p.formStartX, p.formTargetX, e);
          const ny = mix(p.formStartY, p.formTargetY, e);
          const nr = Math.round(mix(p.r, p.formTargetR, e));
          const ng = Math.round(mix(p.g, p.formTargetG, e));
          const nb = Math.round(mix(p.b, p.formTargetB, e));
          const sz = mix(SMALL, SMALL + 0.5, e);
